...
public class GraphDBExample {
    ...
    public static void main(String[] args) {
        ...
        queryString += "SELECT  (AVG (?val) AS ?total) ?actor_name  \n";
        queryString += "WHERE { \n";
        queryString += " ?x act:primaryName ?actor_name.\n" +
                "        ?x act:primaryName ?actor_name."+
                "        ?x act:KnownForTitles ?Movies_actor. "+
                "        ?s domain:runtimeMinutes ?val.";
        queryString += "group by ?name LIMIT 7";
        queryString += "}";
